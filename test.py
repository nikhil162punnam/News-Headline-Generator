...
def prt(label, x):
    print label+':',
    for w in x:
        print idx2word[w].encode('ascii','ignore'),
    print
i = 334
prt('H',Y_train[i])
prt('D',X_train[i])
...
def gensamples(skips=2, k=10, batch_size=batch_size, short=True, temperature=1., use_unk=True):
    ...
    print 'HEAD:',(' '.join(idx2word[w] for w in Y_test[i][:maxlenh])).encode('ascii','ignore')
    print 'DESC:',(' '.join(idx2word[w] for w in X_test[i][:maxlend])).encode('ascii','ignore')
    sys.stdout.flush()
    ...
    for sample, start, score in samples:
        ...
        if short:
            ...
            if distance > -0.6:
                print score, ' '.join(words).encode('ascii','ignore')
    ...
    for b in range(batch_size):
        ...
        for input_idx in flips:
            ...
            if w == empty:  # replace accidental empty with oov
                w = oov0
            if debug and b < debug:
                print '%s => %s'%(idx2word[x_out[b,input_idx]].encode('ascii','ignore'),idx2word[w].encode('ascii','ignore')),
            x_out[b,input_idx] = w
        if debug and b < debug:
            print
    return x_out
